 ...
 */
unsigned char twobit_byte_rna::iterator_to_offset(uint32_t iterator)
{
    return (unsigned char)((3 - (iterator % 4)) * 2);
}

// @todo, offset needs to be second parameter
void twobit_byte_rna::set(unsigned char bit_offset, unsigned char nucleotide)
{
    // bit_offset must be: {0, 2, 4 or 6};
    // nucleotides must be:
    // => T - 00, C - 01, A - 10, G - 11
...
#if DEBUG
    if(bit_offset != 0 and bit_offset != 2 and bit_offset != 4 and bit_offset != 6) {
        throw std::invalid_argument("twobit_byte_rna(bit_offset, ..) must be 0, 2, 4 or 6\n");
    }
#endif //DEBUG
    //set   bit(s): INPUT |= 1 << N;
    //unset bit(s): INPUT &= ~(1 << N);
...
#if DEBUG
    default:
        throw std::invalid_argument("twobit_byte_rna::set(pos, nucleotide) invalid value\n");
        break;
#endif //DEBUG
    }
};
...
// N's are treated as 0, for some weird reason
// this function seems specific for UCSC 2 bit format?! - if so, denote it like that
void twobit_byte_rna::set(char* buffer)
{
    const std::array< unsigned char, 4> bit_offsets = {6, 4, 2, 0};
    for(unsigned char i = 0; i < 4; i++) {
        switch(buffer[i]) {
...
#if DEBUG
        default:
            throw std::invalid_argument("twobit_byte_rna::set(char *) invalid value\n");
            break;
#endif //DEBUG
        }
    }
...
/**
 * @brief fully decodes a twobit byte, not referencing to a hash but allocating a new char*,
 * slower than twobit_byte_rna::get(void) but capable of determining very ends
**/
char *twobit_byte_rna::get(unsigned char length)
{
#if DEBUG
    if(length > 4) {
        throw std::invalid_argument("twobit_byte_rna::get(unsigned char length) -> out of bound: " + std::to_string(length) + "\n");
    }
#endif //DEBUG
    ...
    for(unsigned char i = 0; i < length; i++) { // length = 4: i = 0, 1, 2, 3
        seq[i] = twobit_byte_rna::encode_hash[this->data][i];
    }
    seq[length] = '\0';
...
const char *twobit_byte_rna::get()
{
    return twobit_byte_rna::encode_hash[this->data];
}
