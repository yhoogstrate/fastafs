...
// Resolve a FASTAFS argument to a file path: with from_file the argument is the
// path itself, otherwise it is a database ID. An empty string means not found.
static std::string resolve_fastafs_path(const char *id, bool from_file)
{
    if(from_file) {
...
// Resolve and load a FASTAFS object. Throws when the ID cannot be resolved.
static std::unique_ptr<fastafs> load_fastafs(const char *id, bool from_file)
{
    std::string fname = resolve_fastafs_path(id, from_file);
    if(fname.empty()) {
        throw std::runtime_error("Invalid FASTAFS requested: " + std::string(id));
    }

    auto f = std::make_unique<fastafs>(std::string(id));
    f->load(fname);
    return f;
}
        ...
        if(argc > 1) {
            if(strcmp(argv[1], "--help") == 0 or strcmp(argv[1], "-h") == 0) {
                ...
                if(argc > 3) {
                    ...
                    // -o / --output-file is positional: cache -o <output-file> <input-file>
                    if((strcmp(argv[argc - 3], "-o") == 0) or (strcmp(argv[argc - 3], "--output-file") == 0)) {
                        to_cache = false;
                    }
                ...
                if(argc > 2) {
                    ...
                    std::unique_ptr<fastafs> f = load_fastafs(argv[argc - 1], from_file);
